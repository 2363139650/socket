...
download_dir = r'D:/client/download'  # 下载上传目录全局  .
...
def put_file():
    '''接收文件的内容，以写的方式打开一个新文件并写入来实现下载功能'''
    # 获取包头长
    header_size = struct.unpack('i', soc.send(4))[0]
    # 获得包头
    header_dict = json.loads(soc.send(header_size).decode('GBK'))
    # 解析包头信息 和 文件夹命名
    total_size = header_dict.get('file_size')
    file_path = '%s/%s' % (download_dir, header_dict.get('file_name'))
    print(file_path)
    ...
    with open(file_path, 'rb') as f:  # 必须要用二进制模式打开
        ...
        while send_size < total_size:
            ...
            f.read(lene)
            send_size += len(lene)
            print('总大小：%s  已上传大小:%s' % (total_size, send_size))  # 打印下载进度
